...
use super::{instructions::validate_register_address, memory_alignment::*, trace::{TraceRead, TraceStep}};
...
pub fn op_load_micro_0(instruction: &Instruction, stack: &mut StackTracker, trace_read: &mut TraceRead, micro: u8, base_register_address: u32) -> TraceStep {
    ...
    let (mut imm, rs1, rd, bit_extension) = decode_i_type(stack, &tables, trace_read.opcode, func3, 0x3, None);
    ...
    let post_mem_to_read = add_with_bit_extension(stack, &tables, trace_read.read_1_value, &mut imm, bit_extension );
    ...
    op_load_micro_0_aligned(instruction, &mut if_true, &tables, rd, aligned, alignment, *trace_read, micro, base_register_address);
    op_load_micro_0_missaligned(instruction, &mut if_false, &tables,  rd, aligned, alignment, *trace_read, micro, base_register_address);

    let ret = stack.end_if(if_true, if_false, 11, vec![
        (8, "write_add".to_string()), 
        (8, "write_value".to_string()), 
        (8, "write_pc".to_string()), 
        (1, "write_micro".to_string())  ], 0);
...
pub fn op_load_micro_0_missaligned(instruction: &Instruction, stack: &mut StackTracker, tables: &StackTables, rd: StackVariable, mut aligned: StackVariable, alignment: StackVariable, mut trace_read: TraceRead, _micro: u8, base_register_address: u32) -> TraceStep {
    ...
    let write_value = choose_nibbles(stack, trace_read.read_2_value, alignment, nibs, max_extra, prepad, unsigned, 0);
...
#[allow(clippy::too_many_arguments)]
pub fn op_load_micro_0_aligned(instruction: &Instruction, stack: &mut StackTracker, tables: &StackTables, rd: StackVariable, mut aligned: StackVariable, alignment: StackVariable, mut trace_read: TraceRead, _micro: u8, base_register_address: u32) -> TraceStep {
...
pub fn op_load_micro_1(instruction: &Instruction, stack: &mut StackTracker, trace_read: &mut TraceRead, _micro: u8, base_register_address: u32) -> TraceStep {
    ...
    let mut algined_plus_4 = add_with_bit_extension(stack, &tables, aligned, &mut add_4, bit_extension);
    ...
    stack.move_var(trace_read.read_2_value);
    stack.from_altstack();  //restore alignment
    let write_value = choose_nibbles(stack, trace_read.read_2_value, alignment, nibs, max_extra, pre_pad, unsigned, post);
    stack.rename(write_value, "write_value");
...
pub fn op_load_micro_2(instruction: &Instruction, stack: &mut StackTracker, trace_read: &mut TraceRead, _micro: u8, base_register_address: u32) -> TraceStep {
    ...
    let (imm, rs1, rd, bit_extension) = decode_i_type(stack, &tables, trace_read.opcode, func3, 0x3, None);
    ...
    let write_value = logic_with_bit_extension(stack, &tables, &mut trace_read.read_1_value, &mut trace_read.read_2_value, StackVariable::null(), LogicOperation::Or);
    stack.rename(write_value, "write_value");
...
pub fn op_load_micro_3(instruction: &Instruction, stack: &mut StackTracker, trace_read: &mut TraceRead, _micro: u8, base_register_address: u32) -> TraceStep {
    ...
    stack.equals(&mut trace_read.read_2_add, true, &mut expected_read_2_addr, true);
    let mut expected_read_2_value = stack.number_u32(0);
    stack.equals(&mut trace_read.read_2_value, true, &mut expected_read_2_value, true);
