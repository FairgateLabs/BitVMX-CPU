...
use super::{instructions::ProgramSpec, trace::{TraceRead, TraceStep}};
...
pub fn generate_verification_script(instruction: &Instruction, micro: u8, base_register_address: u32, witness: bool) -> Script {
    ...
    let witness = match witness {
        true => {
            Some(stack.define(8, "witness"))
        },
        false => None,
    };

    let trace_read = TraceRead::define(&mut stack);
    let mut result = execute_step(&mut stack, trace_read, &trace_step, witness, &instruction, micro, program).unwrap();
...
mod tests {
    ...
    fn test_get_key_from_instruction_and_micro() {
        ...
        for (instruction, micro) in sample {
            ...
            let script = generate_verification_script(&instruction, micro, REGISTERS_BASE_ADDRESS, requires_witness(&instruction));
            println!("Instruction: {:?}, Micro: {}, Key: {}, Size: {}", instruction, micro, key, script.len());
        }
