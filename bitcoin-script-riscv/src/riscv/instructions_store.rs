...
pub fn op_store(instruction: &Instruction, stack: &mut StackTracker, trace_read: &mut TraceRead, micro: u8, base_register_address: u32) -> TraceStep {
...
pub fn op_store_micro_0(instruction: &Instruction, stack: &mut StackTracker, trace_read: &mut TraceRead, micro: u32, base_register_address: u32) -> TraceStep {
    ...
    let (bit_extension, mut imm, rs1, rs2) = decode_s_type(stack, &tables, trace_read.opcode, func3);

    //assert micro 0
    stack.move_var(trace_read.micro);
    let mut expected_micro = stack.number(micro);
    ...
    let mem_to_write = add_with_bit_extension(stack, &tables, trace_read.read_1_value, &mut imm, bit_extension );
    ...
    if micro == 4 {
        ...
        aligned = add_with_bit_extension(stack, &tables, aligned, &mut add_4, bit_extension);
    ...
    op_store_sw_aligned(&mut if_true, &tables, *trace_read, aligned, rs2, base_register_address);
    op_store_micro_0_missaligned(&mut if_false, instruction, *trace_read, aligned, rs2, micro, base_register_address);


    let ret = stack.end_if(if_true, if_false, 6, vec![
    ...
    let trace = TraceStep::new(ret[0], ret[1], ret[2], ret[3]);
...
fn op_store_micro_0_missaligned(stack: &mut StackTracker, instruction: &Instruction, mut trace_read: TraceRead, mut aligned: StackVariable, rs2: StackVariable, micro: u32,  base_register_address: u32) {
...
fn op_store_sw_aligned(stack: &mut StackTracker, tables: &StackTables, trace_read: TraceRead, aligned: StackVariable, rs2: StackVariable,  base_register_address: u32) {
...
pub fn op_store_micro_1(instruction: &Instruction, stack: &mut StackTracker, trace_read: &mut TraceRead, micro: u32, base_register_address: u32) -> TraceStep {
    ...
    let trace = TraceStep::new(write_addr, result, write_pc, write_micro);
...
pub fn op_store_micro_2(instruction: &Instruction, stack: &mut StackTracker, trace_read: &mut TraceRead, micro: u32, base_register_address: u32) -> TraceStep {
    ...
    for _ in 0..8 {
        stack.move_var_sub_n(&mut  trace_read.read_1_value, 0);
        stack.move_var_sub_n(&mut  trace_read.read_2_value, 0);
        stack.op_add();
    }

    let write_value = stack.join_count(&mut stack.get_var_from_stack(7), 7);
    stack.rename(write_value, "write_value");
    ...
    let trace = TraceStep::new(write_addr, write_value, write_pc, write_micro);
...
pub fn op_store_micro_3(instruction: &Instruction, stack: &mut StackTracker, trace_read: &mut TraceRead, micro: u32, base_register_address: u32) -> TraceStep {
    ...
    let mut expected_read_2 = stack.number_u32(base_register_address + 0x80);
    stack.equals(&mut trace_read.read_2_add, true, &mut expected_read_2, true);
    move_and_drop(stack, rs2); 
    ...
    let ret = stack.end_if(stack_if_true, stack_if_false, 1, vec![(8, "write_pc".to_string()), (1, "write_micro".to_string())], 0);
    let write_pc = ret[0];
    let write_micro = ret[1];

    
    let trace = TraceStep::new(aligned, write_value, write_pc, write_micro);
