...
pub fn nib_to_bin(stack: &mut StackTracker) {
    for i in [8, 4, 2, 1] {
        ...
        stack.custom(script!{
            ...
            OP_ENDIF
            OP_TOALTSTACK
        }, 1, false, 1, &format!("bit({})", i));
    }
}

//expects the shift ammount and the number to be shifted on the stack
pub fn shift_number(stack: &mut StackTracker, mut to_shift: StackVariable, number: &mut StackVariable, right: bool, msb: bool) -> StackVariable {
...
pub fn sub_1_if_positive(stack: &mut StackTracker) -> StackVariable {
    stack.custom(script!{
        ...
        OP_ENDIF
        OP_TOALTSTACK
    }, 0, true, 0, "sub_1_if_positive").unwrap()
}
...
pub fn shift_lookup(stack: &mut StackTracker) -> StackVariable {
    let mut parts = Vec::new();
    for i in 0..5 { //is not in reverse because the tables itself are in cresendo
...
impl StackLogicTables {
    pub fn new(stack: &mut StackTracker, mask: u8) -> StackLogicTables {
        StackLogicTables {
            and: if mask & LOGIC_MASK_AND == LOGIC_MASK_AND { logic_table(stack, LogicOperation::And)} else { StackVariable::null() },
            or: if mask & LOGIC_MASK_OR == LOGIC_MASK_OR { logic_table(stack, LogicOperation::Or)} else { StackVariable::null() },
            xor: if mask & LOGIC_MASK_XOR == LOGIC_MASK_XOR { logic_table(stack, LogicOperation::Xor)} else { StackVariable::null() },
            mul_mod: if mask & LOGIC_MASK_MUL == LOGIC_MASK_MUL { logic_table(stack, LogicOperation::MulMod)} else { StackVariable::null() },
            mul_quotient: if mask & LOGIC_MASK_MUL == LOGIC_MASK_MUL { logic_table(stack, LogicOperation::MulQuotient)} else { StackVariable::null() },
            lookup: if mask != 0 { half_lookup(stack) } else { StackVariable::null() },
        }
    }
...
impl StackShiftTables {
    pub fn new(stack: &mut StackTracker, mask: u8, left: bool) -> StackShiftTables {
        StackShiftTables {
            shift_0: if mask & 8 == 8 { lshift_table(stack, 0) } else { StackVariable::null() },
            ...
            shift_4: if mask & 8 == 8 { lshift_table(stack, 4) } else { StackVariable::null() },
            shift_lookup: if mask & 8 == 8 { shift_lookup(stack) } else { StackVariable::null() },
        }
    }
...
impl StackTables {
    pub fn new(stack: &mut StackTracker, modulo: bool, quotient: bool, rshift: u8, lshift: u8, logic_mask: u8) -> StackTables {
        StackTables {
            modulo: if modulo { modulo_table(stack, 32)} else { StackVariable::null() },
    ...
    pub fn shift_2nb(&self, stack: &mut StackTracker, right:bool, n: u8) -> StackVariable {
        let (shift, complement) = if right { (&self.rshift, &self.lshift) } else { (&self.lshift, &self.rshift) };
        ...
        stack.rename(result,  &format!("shift_{}{}", if right { "right" } else { "left" }, n));
        result
    }
...
pub fn is_equal_to(stack: &mut StackTracker, value: &StackVariable, than: &StackVariable) -> StackVariable {
...
pub fn is_lower_than(stack: &mut StackTracker, value: &mut StackVariable, than: &mut StackVariable, unsigned: bool) -> StackVariable {
...
pub fn mask_value(stack: &mut StackTracker, mut value: StackVariable, mut mask: StackVariable) -> StackVariable {
...
pub fn left_rotate(stack: &mut StackTracker, value: StackVariable, rotate: StackVariable) -> StackVariable {
...
fn multiply_nib(stack: &mut StackTracker, logic_table: &StackLogicTables, a: StackVariable, b: StackVariable, i: u8, n: u8, add_previous_mod: bool, take_mod_from_alt: bool, add_quot: bool ) {
...
pub fn multiply(stack: &mut StackTracker, a: StackVariable, b: StackVariable, ret_high: bool, ret_low: bool ) -> StackVariable {
    ...
    for k in (0..=(2 * n - 2)).rev() {
        ...
        for i in (end_i..=start_i).rev() {
            let j = k - i;
            multiply_nib(stack, &logic_table, a, b, j, i, total_count > 0, total_count > 0 && count > 0, elements_in_diag > 1 && count > 0 );
            total_count += 1;
            count += 1;
        }
...
pub fn twos_complement(stack: &mut StackTracker, tables: &StackTables, value: StackVariable, size: u32) -> StackVariable {
...
pub fn twos_complement_conditional(stack: &mut StackTracker, tables: &StackTables, value: StackVariable, size: u32) -> StackVariable {
    let (mut stack_true, stack_false) = stack.open_if();
    twos_complement(&mut stack_true, tables, value, size);
    stack.end_if(stack_true, stack_false, 1, vec![(size, "twos_complement".to_string())], 0);
...
pub fn is_negative(stack: &mut StackTracker, value: StackVariable) -> StackVariable {
    ...
    stack.rename(result, &format!("is_neg({})", stack.get_var_name(value)).to_string());
    result
}

pub fn mulh(stack: &mut StackTracker, tables: &StackTables, a: StackVariable, mut b: StackVariable, mulhsu: bool) -> StackVariable {
    ...
    if !mulhsu {
        ...
        stack.custom(script!{ 1 OP_EQUAL }, 1, true, 0, "result_neg").unwrap();            
...
pub fn sign_check(stack: &mut StackTracker, tables: &StackTables, dividend: StackVariable, divisor: StackVariable, quotient: StackVariable, remainder: StackVariable) -> (StackVariable, StackVariable, StackVariable, StackVariable) {
...
pub fn division_and_remainder(stack: &mut StackTracker, tables: &StackTables, dividend: StackVariable, divisor: StackVariable, quotient: StackVariable, remainder: StackVariable, 
        compare: u32, result: Option<u32>, is_rem_check: bool) -> StackVariable {
...
pub fn divu(stack: &mut StackTracker, tables: &StackTables, dividend: StackVariable, divisor: StackVariable, quotient: StackVariable, remainder: StackVariable) -> StackVariable {
    division_and_remainder(stack, tables, dividend, divisor, quotient, remainder, 0, Some(0xFFFF_FFFF), false)
}
